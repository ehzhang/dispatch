...
function getUser(phoneNumber) {
  return Meteor.users.findOne({'profile.phone': phoneNumber});
}
...
function isApproved(phoneNumber) {
  ...
  // TODO check approved as well once there is a way for admins to approve
  // people
  return roles.verified === true; // && roles.approved === true;
}

function setPhone(userId, phoneNumber) {
  // TODO fix this, we might be setting different numbers on server vs client
  if (Meteor.isServer) {
    phoneNumber = canonicalize(phoneNumber);
    sendMessage(phoneNumber, "Text back 'ok' to complete your enrollment.");
  }
  Meteor.users.update({_id: userId}, {$set: {'roles.verified': false, 'profile.phone': phoneNumber}});
}
